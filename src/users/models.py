...
class CustomUser(AbstractUser):
    ...
    profile_picture = models.ImageField(
        upload_to='', default='/static/images/default_profile.jpeg')
    followers = models.ManyToManyField(
        "self", blank=True, related_name='users_following', symmetrical=False)
    following = models.ManyToManyField(
        "self", blank=True, related_name='users_followers', symmetrical=False)
    email = models.EmailField(unique=True, null=False, blank=False)
    full_name = models.CharField(max_length=120, null=False, blank=False)
    username = models.CharField(
        max_length=110, unique=True, null=False, blank=False)
